...
function heroListHTML(){
 ...
 if(heroHallTab==='normal'){
  ...
  body=`<div class="tree-hero-banner"><span>🌳</span><div><b>ผู้พิทักษ์ต้นไม้โลก</b><p>ฮีโร่กิจกรรมแยกจากฮีโร่หลัก • ไม่มีขายในร้านค้า</p><small>ปลดล็อกหนึ่งตัวทุก ๆ 10 ชั้นในโหมดต้นไม้โลก</small></div></div><div class="hero-list hero-list-v14 tree-hero-list">${Object.entries(WORLD_TREE_HERO_DATA).map(([id,h])=>`<button class="hero-card hero-card-v14 tree-exclusive hero-locked" data-hero="${id}" data-hero-source="tree"><span class="floor-badge">ชั้น ${h.floor}</span><span class="hero-card-icon">${h.icon}</span><b>${h.name}</b><em>EXCLUSIVE</em><small>🔒 รางวัลชั้น ${h.floor}</small></button>`).join('')}</div>`;
 }else{
...
function shopHTML(){
 ...
 return `${modalWalletHTML('กระเป๋าของคุณ')}<div class="shop-v15-header"><div><h2>🛒 ตลาดเมฆทอง</h2><p>ฮีโร่ในร้านค้าใช้ Gem • บูสต์ถาวรใช้ Gold</p></div><span class="shop-v15-badge">สินค้าไม่รวมฮีโร่ต้นไม้โลก</span></div>${tabs}<div class="shop-v15-content">${content}</div>`
}
...
(function setupV11MainMenu(){
  ...
  const showMenu=()=>{setActiveScreen('menu');syncMainMenu()};
  const showLobby=()=>{setActiveScreen('lobby');refreshHub()};
  ...
  if(currentStage){showLobby()}else{showMenu()}
})();
