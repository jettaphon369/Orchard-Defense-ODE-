...
(function setupV11MainMenu(){
  ...
  const syncMainMenu=()=>{
    ...
    set('mmLevel',`Lv.${lvl}`);set('mmGold',c.gold||0);set('mmGem',h.gems||0);
  ...
  const showMenu=()=>{menu.style.display='block';hub.style.display='none';if(app)app.style.display='none';window.scrollTo(0,0);syncMainMenu()};
  const showLobby=()=>{menu.style.display='none';hub.style.display='block';if(app)app.style.display='none';window.scrollTo(0,0);refreshHub()};
  ...
  if(currentStage){menu.style.display='none'}else showMenu();
})();
