...
function heroToUI(h){return {icon:h.icon,name:h.name,rarity:h.rarity,role:h.role,floor:h.floor,skill:h.skill,skillDesc:h.skillDesc,stats:h.stats,upgrades:h.upgrades,lore:h.lore,cost:h.cost,source:h.source};}
const HERO_DATA=Object.fromEntries(HERO_REGISTRY.normal.map(h=>[h.id,heroToUI(h)]));
const WORLD_TREE_HERO_DATA=Object.fromEntries(HERO_REGISTRY.tree.map(h=>[h.id,heroToUI(h)]));
const ALL_HERO_DATA={...HERO_DATA,...WORLD_TREE_HERO_DATA};
...
function heroListHTML(){
 ...
 return `${modalWalletHTML('ศาลาฮีโร่')}<div class="hero-hall-head hero-hall-v14"><div><h2>🧙 ศาลาฮีโร่</h2><p>จัดการคอลเลกชัน ดูสกิล และเข้าสนามทดลอง</p></div><button class="sandbox-hall-btn" data-action="sandbox"><span>🧪</span><div><b>สนามทดลองฮีโร่</b><small>ทดลองฮีโร่ครบทุกตัว 30 ตัว • รวมฮีโร่ต้นไม้โลก</small></div></button></div>${heroTabsHTML()}${body}`
}
...
function heroPower(id){const h=ALL_HERO_DATA[id];if(!h)return 0;const hp=Number(String(h.stats.hp||0).replace(/[^0-9.]/g,''))||0;const dmgNums=String(h.stats.damage||0).match(/[0-9.]+/g)||[];let dmg=dmgNums.reduce((a,b)=>a+Number(b),0);if(String(h.stats.damage).includes('×')&&dmgNums.length>1)dmg=Number(dmgNums[0])*Number(dmgNums[1]);const stars=(h.rarity.match(/⭐/g)||[]).length||1;const support=/ซัพพอร์ต|พลังงาน|ควบคุม|แทงก์|ป้องกัน/.test(h.role)?14:0;return Math.max(20,Math.round(hp/20+dmg*.9+stars*12+support))}
function treeOwnedHeroIds(){const normal=campaignSave().ownedHeroes||['peachy'],tree=worldTreeSave().ownedTreeHeroes||[];return Array.from(new Set([...normal,...tree])).filter(id=>ALL_HERO_DATA[id])}
function validTreeTeam(team){const owned=treeOwnedHeroIds();return (Array.isArray(team)?team:[]).filter((id,i,a)=>owned.includes(id)&&ALL_HERO_DATA[id]&&a.indexOf(id)===i).slice(0,6)}
function currentTreeTeam(){const s=worldTreeSave();let team=validTreeTeam(s.team);if(!team.length)team=treeOwnedHeroIds().slice(0,6);return team}
function treeTeamPower(team=currentTreeTeam()){return validTreeTeam(team).reduce((sum,id)=>sum+heroPower(id),0)}
function teamRoleGroup(h){const r=String(h?.role||'').toLowerCase();if(h?.id==='sunny'||r.includes('ซัพพอร์ต')||r.includes('support')||h?.auraAttackSpeed||h?.coreHealEvery)return 'Support';if(r.includes('แทงก์')||r.includes('ป้อมปราการ')||r.includes('แนวหน้า')||r.includes('ผู้พิทักษ์')||h?.damageReduction>=0.18||h?.hp>=450)return 'Tank';if(r.includes('ควบคุม')||r.includes('น้ำแข็ง')||r.includes('ชะลอ')||h?.slow||h?.freezeEvery||h?.stunEvery||h?.knockback)return 'Control';if(r.includes('ผลิตพลังงาน'))return 'Economy';return 'DPS'}
function teamRoleIcon(role){return role==='Tank'?'🛡️':role==='Support'?'✨':role==='Control'?'❄️':role==='Economy'?'☀️':'⚔️'}
function teamRoleLabel(role){return role==='Tank'?'Tank':role==='Support'?'Support':role==='Control'?'Control':role==='Economy'?'Energy':'DPS'}
function teamSummary(team,registry){const roles={Tank:0,Support:0,Control:0,Economy:0,DPS:0};(team||[]).forEach(id=>{const h=registry[id];if(h)roles[teamRoleGroup(h)]++});const warnings=[];if(!roles.Tank)warnings.push('ขาด Tank');if(!roles.Support&&!roles.Economy)warnings.push('ขาด Support/Energy');if(!roles.Control)warnings.push('ไม่มีตัว Control');return{roles,warnings}}
function balancedAutoTeam(ids,registry,max=6){const unique=[...new Set((ids||[]).filter(id=>registry[id]))];const sorted=[...unique].sort((a,b)=>heroPower(b)-heroPower(a));const team=[];const takeRole=role=>{const id=sorted.find(x=>!team.includes(x)&&teamRoleGroup(registry[x])===role);if(id)team.push(id)};takeRole('Tank');takeRole('Support');if(team.length<2)takeRole('Economy');takeRole('Control');sorted.forEach(id=>{if(team.length<max&&!team.includes(id))team.push(id)});return team.slice(0,max)}
...
function worldTreeTeamHTML(floor){const d=treeFloorData(floor),owned=treeOwnedHeroIds();treeTeamDraft=validTreeTeam(treeTeamDraft.length?treeTeamDraft:currentTreeTeam());const power=treeTeamPower(treeTeamDraft),status=treePowerStatus(power,d.power),summary=teamSummary(treeTeamDraft,ALL_HERO_DATA);const slots=Array.from({length:6},(_,i)=>{const id=treeTeamDraft[i],h=id?ALL_HERO_DATA[id]:null;return `<div class="tree-team-slot ${h?'filled':''}"><span>${h?h.icon:'＋'}</span><small>${h?h.name:`ช่อง ${i+1}`}</small>${h?`<em>${teamRoleIcon(teamRoleGroup(h))}</em>`:''}</div>`}).join('');const cards=owned.map(id=>{const h=ALL_HERO_DATA[id],selected=treeTeamDraft.includes(id),role=teamRoleGroup(h);return `<button class="tree-team-hero ${selected?'selected':''}" data-tree-team-hero="${id}"><span>${h.icon}</span><b>${h.name}</b><small class="team-role">${teamRoleIcon(role)} ${teamRoleLabel(role)} • พลัง ${heroPower(id)}</small><small class="team-skill"><strong>${h.skill||'สกิล'}</strong> — ${h.skillDesc||h.role||''}</small><em>${selected?'✓ ในทีม':'แตะเพื่อเลือก'}</em></button>`}).join('');const warn=summary.warnings.length?`<div class="team-warning">⚠️ ${summary.warnings.join(' • ')}</div>`:`<div class="team-warning good">✅ ทีมมีบทบาทหลักค่อนข้างครบ</div>`;const roleStrip=Object.entries(summary.roles).filter(([,n])=>n).map(([r,n])=>`<span>${teamRoleIcon(r)} ${teamRoleLabel(r)} ${n}</span>`).join('');return `<div class="tree-team-screen"><button class="tree-team-back" data-tree-team-back>← กลับหน้าชั้น</button><section class="tree-team-head"><div><small>🌳 ต้นไม้โลก</small><h2>จัดทีมก่อนเข้าชั้น ${floor}</h2><p>เลือกฮีโร่สูงสุด 6 ตัว • ตรวจพลังและบทบาทก่อนเริ่ม</p><div class="team-role-strip">${roleStrip||'<span>ยังไม่ได้เลือกฮีโร่</span>'}</div>${warn}</div><div class="tree-team-comparison ${status.cls}"><div><small>พลังทีม</small><strong>⚔ ${power}</strong></div><i>VS</i><div><small>พลังแนะนำ</small><strong>⚔ ${d.power}</strong></div><b>${status.icon} ${status.label}</b></div></section><section class="tree-team-slots">${slots}</section><div class="tree-team-count">เลือกแล้ว <b>${treeTeamDraft.length}/6</b> ตัว</div><section class="tree-team-roster">${cards||'<p>ยังไม่มีฮีโร่ที่ใช้ได้</p>'}</section><div class="tree-team-actions"><button data-tree-team-auto>✨ จัดทีมสมดุลอัตโนมัติ</button><button class="tree-team-confirm" data-tree-team-confirm="${floor}" ${treeTeamDraft.length?'':'disabled'}>⚔ ยืนยันและเข้าด่าน</button></div></div>`}
...
function campaignTeamHTML(stage){const meta=STAGE_META[stage]||STAGE_META[1],owned=(campaignSave().ownedHeroes||['peachy']).filter(id=>HERO_DATA[id]);campaignTeamDraft=validCampaignTeam(campaignTeamDraft.length?campaignTeamDraft:currentCampaignTeam());const recommended=campaignStagePower(stage),power=campaignTeamPower(campaignTeamDraft),status=treePowerStatus(power,recommended),summary=teamSummary(campaignTeamDraft,HERO_DATA);const slots=Array.from({length:6},(_,i)=>{const id=campaignTeamDraft[i],h=id?HERO_DATA[id]:null;return `<div class="tree-team-slot ${h?'filled':''}"><span>${h?h.icon:'＋'}</span><small>${h?h.name:`ช่อง ${i+1}`}</small>${h?`<em>${teamRoleIcon(teamRoleGroup(h))}</em>`:''}</div>`}).join('');const cards=owned.map(id=>{const h=HERO_DATA[id],selected=campaignTeamDraft.includes(id),role=teamRoleGroup(h);return `<button class="tree-team-hero ${selected?'selected':''}" data-campaign-team-hero="${id}"><span>${h.icon}</span><b>${h.name}</b><small class="team-role">${teamRoleIcon(role)} ${teamRoleLabel(role)} • พลัง ${heroPower(id)}</small><small class="team-skill"><strong>${h.skill||'สกิล'}</strong> — ${h.skillDesc||h.role||''}</small><em>${selected?'✓ ในทีม':'แตะเพื่อเลือก'}</em></button>`}).join('');const warn=summary.warnings.length?`<div class="team-warning">⚠️ ${summary.warnings.join(' • ')}</div>`:`<div class="team-warning good">✅ ทีมมีบทบาทหลักค่อนข้างครบ</div>`;const roleStrip=Object.entries(summary.roles).filter(([,n])=>n).map(([r,n])=>`<span>${teamRoleIcon(r)} ${teamRoleLabel(r)} ${n}</span>`).join('');return `<div class="tree-team-screen campaign-team-screen"><button class="tree-team-back" data-campaign-team-back>← กลับแผนที่</button><section class="tree-team-head"><div><small>${meta.icon} แผนที่เกาะลอยฟ้า</small><h2>จัดทีมก่อนเข้าด่าน ${stage}</h2><p>${meta.name} • ${meta.waves} เวฟ • เลือกฮีโร่สูงสุด 6 ตัว</p><div class="team-role-strip">${roleStrip||'<span>ยังไม่ได้เลือกฮีโร่</span>'}</div>${warn}</div><div class="tree-team-comparison ${status.cls}"><div><small>พลังทีม</small><strong>⚔ ${power}</strong></div><i>VS</i><div><small>พลังแนะนำ</small><strong>⚔ ${recommended}</strong></div><b>${status.icon} ${status.label}</b></div></section><section class="tree-team-slots">${slots}</section><div class="tree-team-count">เลือกแล้ว <b>${campaignTeamDraft.length}/6</b> ตัว</div><section class="tree-team-roster">${cards||'<p>ยังไม่มีฮีโร่ที่ใช้ได้</p>'}</section><div class="tree-team-actions"><button data-campaign-team-auto>✨ จัดทีมสมดุลอัตโนมัติ</button><button class="tree-team-confirm" data-campaign-team-confirm="${stage}" ${campaignTeamDraft.length?'':'disabled'}>⚔ ยืนยันและเข้าด่าน</button></div></div>`}
