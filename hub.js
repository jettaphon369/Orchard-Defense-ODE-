...
function hubSave(){let h;try{h=JSON.parse(localStorage.getItem(HUB_KEY))||{}}catch{h={}};h.gems=Number(h.gems??25);h.forge=h.forge||0;h.energyBonus=h.energyBonus||0;h.skinOwned=!!h.skinOwned;h.login=h.login||{lastClaim:'',day:0};return h}
...
function refreshHub(){const c=campaignSave(),h=hubSave(),star=Object.values(c.stars).reduce((a,b)=>a+b,0);saveCampaign(c);$h('#goldText').textContent=c.gold;$h('#gemText').textContent=h.gems;$h('#levelText').textContent=`Lv.${1+Math.floor(star/2)}`;const ready=questDefs(c).filter(q=>q.now>=q.target&&!c.claimedQuests[today()+q.id]).length;$h('#questBadge').textContent=ready?`รับได้ ${ready}`:'กำลังทำ';$h('#shopBadge').textContent='ฮีโร่พิเศษ'}
...
function openFeature(place){if(place==='adventure'){movePlayer(place,()=>{$h('#map').classList.add('show');renderMap()});return}movePlayer(place,()=>{const m=$h('#buildingModal');$h('#featureContent').innerHTML=featureHTML(place);m.classList.add('show');bindFeature(place)})}
function shopHTML(){const c=campaignSave(),h=hubSave();const special=Object.entries(SHOP_HEROES).map(([id,price])=>{const x=HERO_DATA[id],owned=c.ownedHeroes.includes(id);return `<div class="product-card ${owned?'owned':''}"><div class="product-head"><b>${x.icon} ${x.name}</b><strong>💎 ${price}</strong></div><p class="product-desc">${x.role} — ${x.skillDesc}</p><div class="product-effect">ฮีโร่พิเศษ • ซื้อแล้วใช้วางในด่านได้ทันที</div><button class="buy-button" data-hero-buy="${id}" ${owned?'disabled':''}>${owned?'ครอบครองแล้ว ✓':'ซื้อด้วยเพชร'}</button></div>`}).join('');return `<h2>🛒 ร้านค้าเมฆทอง</h2><p>Gold ใช้อัปเกรดเมือง • Gem ใช้ซื้อฮีโร่พิเศษ</p><div class="shop-list"><div class="product-card ${h.energyBonus>=10?'owned':''}"><div class="product-head"><b>☀️ พลังงานเริ่มต้น +10</b><strong>100 Gold</strong></div><p class="product-desc">เริ่มทุกด่านด้วยพลังงานเพิ่ม 10 หน่วย</p><button class="buy-button" data-buy="energy" ${h.energyBonus>=10?'disabled':''}>${h.energyBonus>=10?'ซื้อแล้ว ✓':'ซื้อสินค้า'}</button></div>${special}</div>`}
