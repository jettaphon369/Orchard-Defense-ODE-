...
class BattleScene extends Phaser.Scene{
  ...
  endGame(win){
    ...
    if(win){
      ...
      if(STAGE.heroUnlock&&!save.ownedHeroes.includes(STAGE.heroUnlock))save.ownedHeroes.push(STAGE.heroUnlock);
    ...
    this.add.text(480,270,`กำจัด ${state.kills} ตัว • Core ${Math.max(0,Math.ceil(this.baseHP))}%${win?` • +${STAGE.reward} Gold • +${STAGE.gems||0} Gem • ปลดล็อก ${STAGE.heroUnlock?UNIT_DEFS.find(x=>x.id===STAGE.heroUnlock)?.name:''}`:''}`,{font:'bold 20px Arial',color:'#dceaff'}).setOrigin(.5).setDepth(61);
