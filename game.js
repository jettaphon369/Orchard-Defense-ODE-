...
class BattleScene extends Phaser.Scene{
  ...
  selectCard(id){const def=UNIT_DEFS.find(x=>x.id===id);if(!def||state.energy<def.cost)return;this.closeUnitMenu();state.selected=id;document.querySelectorAll('.card').forEach(x=>x.classList.toggle('selected',x.dataset.id===id));this.ghostSprite.setTexture(id).setDisplaySize(64,64);this.ghost.setVisible(false)}
  ...
  placeUnit(id,r,c,x,y){const def=UNIT_DEFS.find(d=>d.id===id);if(state.energy<def.cost)return;state.energy-=def.cost;const s=this.add.image(x,y,id).setDisplaySize(64,64).setDepth(10);s.setScale(0);this.tweens.add({targets:s,scale:.67,duration:220,ease:'Back.Out'});const hp=this.add.graphics().setDepth(12);const levelText=this.add.text(x+24,y+23,'Lv.1',{font:'bold 11px Arial',color:'#ffffff',backgroundColor:'#17375f',padding:{x:4,y:2},stroke:'#07152a',strokeThickness:2}).setOrigin(.5).setDepth(14);const u={def,r,c,x,y,s,hp,levelText,maxHp:def.hp,hpVal:def.hp,next:400,age:0,level:1};this.units.push(u);this.closeUnitMenu();state.cooldowns[id]=def.cool;state.selected=null;this.ghost.setVisible(false);document.querySelectorAll('.card').forEach(x=>x.classList.remove('selected'));this.burst(x,y,0x83f4ff,12);this.beep(520,.07);this.updateHud()}
  ...
  endGame(win){
    ...
    if(win){
      ...
      let save;
      try{save=JSON.parse(localStorage.getItem('orchardCampaignV1'))||{unlocked:1,stars:{}}}
      catch{save={unlocked:1,stars:{}}}
      save.unlocked=Math.max(save.unlocked,Math.min(3,CURRENT_STAGE+1));
    ...
    this.add.rectangle(480,270,960,540,0x041027,.88).setDepth(60);
    this.add.text(480,168,win?'🏆 ผ่านด่านแล้ว!':'💔 สวนถูกทำลาย',{font:'bold 42px Arial',color:win?'#ffe276':'#ff7280',stroke:'#051026',strokeThickness:8}).setOrigin(.5).setDepth(61);
    this.add.text(480,225,win?`${STAGE.name}  ${'⭐'.repeat(stars)}`:`ด่าน ${CURRENT_STAGE} • ${STAGE.name}`,{font:'bold 27px Arial',color:'#fff'}).setOrigin(.5).setDepth(61);
    this.add.text(480,270,`กำจัด ${state.kills} ตัว • Core ${Math.max(0,Math.ceil(this.baseHP))}%${win?` • +${STAGE.reward} Gold`:''}`,{font:'bold 20px Arial',color:'#dceaff'}).setOrigin(.5).setDepth(61);

    const makeButton=(x,label,color,onClick,width=180)=>{
      const btn=this.add.rectangle(x,355,width,56,color).setStrokeStyle(3,0xffffff).setInteractive({useHandCursor:true}).setDepth(61);
      this.add.text(x,355,label,{font:'bold 21px Arial',color:'#fff'}).setOrigin(.5).setDepth(62);
      btn.on('pointerdown',onClick);
      return btn;
    };

    if(win && CURRENT_STAGE<3){
