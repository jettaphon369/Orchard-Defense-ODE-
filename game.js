...
class BattleScene extends Phaser.Scene{
  ...
  drawBaseBar(){
    ...
    this.baseBar.clear()
      ...
      .fillStyle(this.baseHP>35?0x47d65b:0xef4f55).fillRoundedRect(162,32,134*this.baseHP/100,8,4);
  ...
  placeUnit(id,r,c,x,y){
    ...
    const u={def,r,c,x,y,s,hp,levelText,maxHp:def.hp,hpVal:def.hp,next:400,age:0,level:1};
  ...
  showVictoryRewards(rewards){
    const panelY=326;
    const wrap=this.add.container(480,panelY).setDepth(64);
    const glow=this.add.circle(0,-76,42,0xffdc73,.24);
    const chest=this.add.text(0,-76,'🎁',{font:'48px Arial'}).setOrigin(.5);
    const bg=this.add.rectangle(0,0,700,178,0x0b264d,.96).setStrokeStyle(4,0x7ec9ff,1);
    const inner=this.add.rectangle(0,0,672,150,0x123566,.52).setStrokeStyle(2,0xffffff,.14);
    const title=this.add.text(0,-54,'✨ สมบัติชัยชนะ',{font:'bold 28px Arial',color:'#ffe98c',stroke:'#071329',strokeThickness:5}).setOrigin(.5);
    const subtitle=this.add.text(0,-26,'รับของรางวัลสุดพิเศษจากการปกป้องสวนลอยฟ้า',{font:'16px Arial',color:'#dceaff'}).setOrigin(.5);
    wrap.add([bg,inner,glow,chest,title,subtitle]);

    this.tweens.add({targets:glow,scale:1.35,alpha:.08,duration:900,yoyo:true,repeat:-1});
    this.tweens.add({targets:chest,angle:{from:-6,to:6},duration:220,yoyo:true,repeat:2});
    this.tweens.add({targets:wrap,scale:{from:.96,to:1},alpha:{from:0,to:1},duration:260,ease:'Back.Out'});
    this.burst(480,panelY-76,0xffdc73,20);

    const positions=rewards.length===2?[-105,105]:[-210,0,210];
    rewards.forEach((reward,index)=>{
      const x=positions[index]||0;
      const card=this.add.container(x,28).setScale(.35).setAlpha(0);
      const aura=this.add.circle(0,0,62,reward.glow,.22);
      const shell=this.add.rectangle(0,0,178,114,reward.bg,1).setStrokeStyle(3,reward.stroke,1);
      const topTag=this.add.rectangle(0,-37,132,24,reward.accent,1).setStrokeStyle(2,0xffffff,.25);
      const topText=this.add.text(0,-37,reward.badge,{font:'bold 12px Arial',color:'#fff'}).setOrigin(.5);
      const iconRing=this.add.circle(0,-6,26,0xffffff,.13).setStrokeStyle(2,reward.stroke,.7);
      const icon=this.add.text(0,-7,reward.icon,{font:'30px Arial'}).setOrigin(.5);
      const amount=this.add.text(0,20,reward.amount,{font:'bold 24px Arial',color:'#fff5cc',stroke:'#071329',strokeThickness:4}).setOrigin(.5);
      const label=this.add.text(0,45,reward.label,{font:'bold 16px Arial',color:'#ffffff'}).setOrigin(.5);
      const sub=this.add.text(0,63,reward.sub,{font:'13px Arial',color:'#d8ebff'}).setOrigin(.5);
      card.add([aura,shell,topTag,topText,iconRing,icon,amount,label,sub]);
      wrap.add(card);
      this.tweens.add({targets:aura,scale:1.22,alpha:.12,duration:1000,yoyo:true,repeat:-1,delay:index*120});
      this.tweens.add({targets:card,alpha:1,scale:1,y:0,duration:460,delay:180+index*120,ease:'Back.Out'});
      this.tweens.add({targets:icon,angle:{from:-7,to:7},duration:300,yoyo:true,repeat:1,delay:280+index*120});
    });
  }
  ...
  endGame(win){
    ...
    let unlockedHeroDef=STAGE.heroUnlock?UNIT_DEFS.find(x=>x.id===STAGE.heroUnlock):null;
    if(win){
      ...
      save.unlocked=Math.max(save.unlocked||1,Math.min(10,CURRENT_STAGE+1));save.stars=save.stars||{};save.stars[CURRENT_STAGE]=Math.max(save.stars[CURRENT_STAGE]||0,stars);save.gold=(save.gold||0)+STAGE.reward;save.ownedHeroes=Array.from(new Set(save.ownedHeroes?.length?save.ownedHeroes:['peachy']));
      ...
      save.stats=save.stats||{kills:0,unitsPlaced:0,upgrades:0,stagesCleared:0,bosses:0};save.stats.stagesCleared++;
      const d=new Date().toISOString().slice(0,10);if(save.daily?.date!==d)save.daily={date:d,kills:0,unitsPlaced:0,upgrades:0,stagesCleared:0};save.daily.stagesCleared++;
      localStorage.setItem('orchardCampaignV1',JSON.stringify(save));
      let hub;try{hub=JSON.parse(localStorage.getItem('orchardHubV1'))||{}}catch{hub={}};hub.gems=Number(hub.gems||0)+Number(STAGE.gems||0);localStorage.setItem('orchardHubV1',JSON.stringify(hub));
    }

    this.add.rectangle(480,270,960,540,0x041027,.9).setDepth(60);
    this.add.text(480,112,win?'🏆 ผ่านด่านแล้ว!':'💔 สวนถูกทำลาย',{font:'bold 42px Arial',color:win?'#ffe276':'#ff7280',stroke:'#051026',strokeThickness:8}).setOrigin(.5).setDepth(61);
    this.add.text(480,158,win?`${STAGE.name}  ${'⭐'.repeat(stars)}`:`ด่าน ${CURRENT_STAGE} • ${STAGE.name}`,{font:'bold 26px Arial',color:'#fff'}).setOrigin(.5).setDepth(61);
    this.add.text(480,196,`กำจัด ${state.kills} ตัว • Core ${Math.max(0,Math.ceil(this.baseHP))}%`,{font:'bold 20px Arial',color:'#dceaff'}).setOrigin(.5).setDepth(61);

    if(win){
      const rewards=[
        {badge:'GOLD',icon:'🪙',amount:`+${STAGE.reward}`,label:'เหรียญทอง',sub:'ใช้ซื้อฮีโร่และไอเทม',bg:0x6b4e12,stroke:0xffd86b,accent:0xd69a1e,glow:0xffd86b},
        {badge:'GEM',icon:'💎',amount:`+${STAGE.gems||0}`,label:'เพชรฟ้า',sub:'สกุลเงินพิเศษประจำเกาะ',bg:0x113b64,stroke:0x74e7ff,accent:0x288fc7,glow:0x74e7ff}
      ];
      if(unlockedHeroDef){
        rewards.push({badge:unlockedHeroNew?'NEW HERO':'HERO',icon:unlockedHeroDef.icon,amount:unlockedHeroDef.name,label:unlockedHeroNew?'ปลดล็อกฮีโร่ใหม่':'ฮีโร่ประจำด่าน',sub:unlockedHeroNew?'เพิ่มเข้าเด็คเรียบร้อยแล้ว':'สะสมแล้ว เล่นซ้ำเพื่อเก็บรางวัล',bg:0x3e215d,stroke:0xffa0ff,accent:0xa95cff,glow:0xff9dff});
      }
      this.showVictoryRewards(rewards);
    }

    const makeButton=(x,label,color,onClick,width=180)=>{
      const btn=this.add.rectangle(x,458,width,56,color).setStrokeStyle(3,0xffffff).setInteractive({useHandCursor:true}).setDepth(61);
      btn.on('pointerover',()=>btn.setScale(1.04));
      btn.on('pointerout',()=>btn.setScale(1));
      this.add.text(x,458,label,{font:'bold 21px Arial',color:'#fff'}).setOrigin(.5).setDepth(62);
      btn.on('pointerdown',onClick);
      return btn;
    };

    if(win && CURRENT_STAGE<10){
      makeButton(275,'เล่นอีกครั้ง',0x2f90c9,()=>location.reload(),170);
      makeButton(480,'ด่านถัดไป ▶',0xd58a24,()=>{
        localStorage.setItem('orchardCurrentStage',String(CURRENT_STAGE+1));
        location.reload();
      },190);
      makeButton(685,'กลับแผนที่',0x388d55,()=>window.openCampaignMap(),170);
    }else{
      makeButton(365,'เล่นอีกครั้ง',0x2f90c9,()=>location.reload(),190);
      makeButton(595,win?'กลับแผนที่':'เลือกด่าน',0x388d55,()=>window.openCampaignMap(),190);
    }
  }
