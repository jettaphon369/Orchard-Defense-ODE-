...
const state = {energy:150,wave:1,maxWave:10,kills:0,selected:null,speed:1,paused:false,cooldowns:{}};
...
class BattleScene extends Phaser.Scene{
  ...
  tickSecond(){if(state.paused)return;state.energy=Math.min(999,state.energy+8);for(const id in state.cooldowns)state.cooldowns[id]=Math.max(0,state.cooldowns[id]-1000);this.updateHud()}
  ...
  updateEnemies(dt){for(const e of [...this.enemies]){e.atk-=dt;e.s.setAngle(Math.sin(this.time.now/140+e.y)*3);e.s.setY(e.y+Math.sin(this.time.now/180+e.x)*2);e.bar.clear().fillStyle(0x321725,.9).fillRect(e.x-29,e.y-39,58,7).fillStyle(e.kind==='dragon'?0xffb32d:0xef4b61).fillRect(e.x-28,e.y-38,56*Math.max(0,e.hp/e.maxHp),5);if(e.hp<=0){this.killEnemy(e);continue}e.s.setTint(e.slow>0?0x9befff:0xffffff);e.slow=Math.max(0,e.slow-dt);const blocker=this.units.filter(u=>u.r===e.row&&u.x<e.x&&e.x-u.x<50).sort((a,b)=>b.x-a.x)[0];if(blocker){if(e.atk<=0){blocker.hpVal-=e.damage;e.atk=850;this.tweens.add({targets:blocker.s,x:blocker.x-4,yoyo:true,duration:80});this.floatText(blocker.x,blocker.y-25,`-${e.damage}`,'#ff8a8a')}}else{e.x-=e.speed*(e.slow>0?.45:1)*dt/1000;e.s.setX(e.x)}if(e.x<105){this.baseHP-=e.kind==='dragon'?20:8;this.drawBaseBar();this.removeEnemy(e);this.cameras.main.shake(160,.008);if(this.baseHP<=0)this.endGame(false)}}}
  updateProjectiles(dt){for(const p of [...this.projectiles]){if(!p.target||!p.target.s.active){p.orb.destroy();this.projectiles=this.projectiles.filter(x=>x!==p);continue}const dx=p.target.x-p.x,dy=p.target.y-p.y,d=Math.hypot(dx,dy);const step=p.speed*dt/1000;if(d<=step+9){this.hitEnemy(p.target,p.damage,p.slow);this.burst(p.target.x,p.target.y,p.slow?0x72e8ff:0xffdb6d,5);p.orb.destroy();this.projectiles=this.projectiles.filter(x=>x!==p)}else{p.x+=dx/d*step;p.y+=dy/d*step;p.orb.setPosition(p.x,p.y)}}}
  ...
  killEnemy(e){state.kills++;state.energy=Math.min(999,state.energy+(e.kind==='dragon'?100:e.kind==='orc'?18:10));this.burst(e.x,e.y,e.kind==='dragon'?0xff516f:0xa78bff,e.kind==='dragon'?30:12);this.tweens.add({targets:e.s,alpha:0,scale:1.4,angle:25,duration:220,onComplete:()=>e.s.destroy()});e.bar.destroy();this.enemies=this.enemies.filter(x=>x!==e);this.updateHud()}
  removeEnemy(e){e.s.destroy();e.bar.destroy();this.enemies=this.enemies.filter(x=>x!==e)}
  ...
  updateHud(){state.energy=Math.floor(state.energy);$('#energyText').textContent=`☀️ ${state.energy}`;$('#waveText').textContent=`🌊 ${state.wave}/${state.maxWave}`;this.updateCards()}
