...
const STAGES={
  1:{id:1,name:'สวนลอยฟ้า',maxWave:8,energy:175,sky:0x7ee8f4,groundA:0x6bb85b,groundB:0x72bf5f,border:0xf3e7ac,boss:'ลูกมังกร',bossIcon:'🐉',enemyHp:1,enemySpeed:1,reward:100},
  2:{id:2,name:'ป่าเวทมนตร์',maxWave:10,energy:160,sky:0x7798b8,groundA:0x315d48,groundB:0x3e7257,border:0x8fb28b,boss:'ต้นไม้โบราณ',bossIcon:'🌳',enemyHp:1.22,enemySpeed:1.06,reward:200},
  3:{id:3,name:'ภูเขาไฟลอยฟ้า',maxWave:12,energy:150,sky:0xb65d55,groundA:0x684235,groundB:0x7b4a36,border:0xd29a62,boss:'มังกรเพลิง',bossIcon:'🔥',enemyHp:1.48,enemySpeed:1.12,reward:300}
};
const CURRENT_STAGE=Math.max(1,Math.min(3,Number(localStorage.getItem('orchardCurrentStage')||1)));
const STAGE=STAGES[CURRENT_STAGE];

const state = {energy:STAGE.energy,wave:1,maxWave:STAGE.maxWave,kills:0,selected:null,speed:1,paused:false,cooldowns:{},skill:0,sound:true};
...
class BattleScene extends Phaser.Scene{
  ...
  drawBaseBar(){const hp=Math.max(0,Math.ceil(this.baseHP));this.baseBar.clear().fillStyle(0x0c1b38,.9).fillRoundedRect(22,17,320,40,10).lineStyle(2,0x5d80b3).strokeRoundedRect(22,17,320,40,10).fillStyle(0xffffff,.95).fillRoundedRect(160,30,138,12,5).fillStyle(this.baseHP>35?0x47d65b:0xef4f55).fillRoundedRect(162,32,134*this.baseHP/100,8,4);this.baseLabel?.destroy();this.basePercent?.destroy();this.baseLabel=this.add.text(35,28,'🏰 สวนลอยฟ้า',{font:'bold 15px Arial',color:'#ffffff',stroke:'#071329',strokeThickness:2}).setDepth(21);this.basePercent=this.add.text(322,28,`${hp}%`,{font:'bold 13px Arial',color:'#ffffff',stroke:'#071329',strokeThickness:2}).setOrigin(.5,0).setDepth(21)}
  ...
  updateEnemies(dt){for(const e of [...this.enemies]){e.atk-=dt;e.s.setAngle(Math.sin(this.time.now/140+e.y)*3);e.s.setY(e.y+Math.sin(this.time.now/180+e.x)*2);if(e.kind==='dragon'){const ratio=Math.max(0,Math.min(1,e.hp/e.maxHp));const track=$('#bossTrack');const hpPct=Math.round(ratio*100);track.style.setProperty('--boss-hp',`${hpPct}%`);track.setAttribute('aria-valuenow',String(hpPct))}e.bar.clear().fillStyle(0x321725,.9).fillRect(e.x-29,e.y-39,58,7).fillStyle(e.kind==='dragon'?0xffb32d:0xef4b61).fillRect(e.x-28,e.y-38,56*Math.max(0,e.hp/e.maxHp),5);if(e.hp<=0){this.killEnemy(e);continue}e.s.setTint(e.slow>0?0x9befff:0xffffff);e.slow=Math.max(0,e.slow-dt);const blocker=this.units.filter(u=>u.r===e.row&&u.x<e.x&&e.x-u.x<50).sort((a,b)=>b.x-a.x)[0];if(blocker){if(e.atk<=0){blocker.hpVal-=e.damage;e.atk=850;this.tweens.add({targets:blocker.s,x:blocker.x-4,yoyo:true,duration:80});this.floatText(blocker.x,blocker.y-25,`-${e.damage}`,'#ff8a8a')}}else{e.x-=e.speed*(e.slow>0?.45:1)*dt/1000;e.s.setX(e.x)}if(e.x<105){this.baseHP-=e.kind==='dragon'?20:8;this.drawBaseBar();this.removeEnemy(e);this.cameras.main.shake(160,.008);if(this.baseHP<=0)this.endGame(false)}}}
  ...
  endGame(win){if(state.paused)return;state.paused=true;let stars=0;if(win){stars=1+(this.baseHP>=60?1:0)+(this.baseHP>=90?1:0);let save;try{save=JSON.parse(localStorage.getItem('orchardCampaignV1'))||{unlocked:1,stars:{}}}catch{save={unlocked:1,stars:{}}}save.unlocked=Math.max(save.unlocked,Math.min(3,CURRENT_STAGE+1));save.stars=save.stars||{};save.stars[CURRENT_STAGE]=Math.max(save.stars[CURRENT_STAGE]||0,stars);save.gold=(save.gold||0)+STAGE.reward;localStorage.setItem('orchardCampaignV1',JSON.stringify(save));}this.add.rectangle(480,270,960,540,0x041027,.88).setDepth(60);this.add.text(480,178,win?'🏆 ผ่านด่านแล้ว!':'💔 สวนถูกทำลาย',{font:'bold 42px Arial',color:win?'#ffe276':'#ff7280',stroke:'#051026',strokeThickness:8}).setOrigin(.5).setDepth(61);this.add.text(480,235,win?`${STAGE.name}  ${'⭐'.repeat(stars)}`:`ด่าน ${CURRENT_STAGE} • ${STAGE.name}`,{font:'bold 27px Arial',color:'#fff'}).setOrigin(.5).setDepth(61);this.add.text(480,280,`กำจัด ${state.kills} ตัว • Core ${Math.max(0,Math.ceil(this.baseHP))}%${win?` • +${STAGE.reward} Gold`:''}`,{font:'bold 20px Arial',color:'#dceaff'}).setOrigin(.5).setDepth(61);const retry=this.add.rectangle(365,355,190,56,0x2f90c9).setStrokeStyle(3,0xffffff).setInteractive().setDepth(61);this.add.text(365,355,'เล่นอีกครั้ง',{font:'bold 22px Arial',color:'#fff'}).setOrigin(.5).setDepth(62);retry.on('pointerdown',()=>location.reload());const map=this.add.rectangle(595,355,190,56,0x388d55).setStrokeStyle(3,0xffffff).setInteractive().setDepth(61);this.add.text(595,355,'กลับแผนที่',{font:'bold 22px Arial',color:'#fff'}).setOrigin(.5).setDepth(62);map.on('pointerdown',()=>window.openCampaignMap());}
