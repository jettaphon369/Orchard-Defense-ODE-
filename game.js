...
{id:'berry',name:'Berry Mage',icon:'🫐',texture:'berry',cost:155,role:'เมจ',hp:110,damage:38,rate:1300,range:900,cool:5500},
...
{id:'bloom',name:'Bloom Fairy',icon:'🌺',texture:'bloom',cost:165,role:'ยิงเร็ว',hp:100,damage:20,rate:750,range:900,cool:6000},
...
{id:'thunderpear',name:'Thunder Pear',icon:'🍐',texture:'thunderpear',cost:190,role:'สายฟ้า',hp:115,damage:30,rate:850,range:900,cool:6500},
...
class BattleScene extends Phaser.Scene{
  ...
  fireProjectile(u,target,delay=0){this.time.delayedCall(delay,()=>{if(!u.s.active||!target.s.active)return;const color=u.def.slow?0x6de8ff:0xffd55b;const orb=this.add.circle(u.x+22,u.y-5,6,color).setDepth(15);this.projectiles.push({orb,x:u.x+22,y:u.y-5,target,damage:u.def.damage,speed:430,slow:u.def.slow,slowFactor:u.def.slowFactor||.45,freezeEvery:u.def.freezeEvery||0,freezeMs:u.def.freezeMs||0,source:u})})}
  ...
  updateEnemies(dt){for(const e of [...this.enemies]){e.atk-=dt;e.s.setAngle(Math.sin(this.time.now/140+e.y)*3);e.s.setY(e.y+Math.sin(this.time.now/180+e.x)*2);if(e.kind==='dragon')this.drawBossBar(e);e.bar.clear().fillStyle(0x321725,.9).fillRect(e.x-29,e.y-39,58,7).fillStyle(e.kind==='dragon'?0xffb32d:0xef4b61).fillRect(e.x-28,e.y-38,56*Math.max(0,e.hp/e.maxHp),5);if(e.hp<=0){this.killEnemy(e);continue}e.s.setTint(e.slow>0?0x9befff:0xffffff);e.slow=Math.max(0,e.slow-dt);e.freeze=Math.max(0,(e.freeze||0)-dt);const blocker=this.units.filter(u=>u.r===e.row&&u.x<e.x&&e.x-u.x<50).sort((a,b)=>b.x-a.x)[0];if(blocker){if(e.atk<=0){blocker.hpVal-=e.damage;e.atk=850;this.tweens.add({targets:blocker.s,x:blocker.x-4,yoyo:true,duration:80});this.floatText(blocker.x,blocker.y-25,`-${e.damage}`,'#ff8a8a')}}else{e.x-=e.speed*(e.freeze>0?0:(e.slow>0?(e.slowFactor||.45):1))*dt/1000;e.s.setX(e.x)}if(e.x<105){if(!SANDBOX_MODE)this.baseHP-=e.kind==='dragon'?20:8;this.drawBaseBar();this.removeEnemy(e);this.cameras.main.shake(160,.008);if(!SANDBOX_MODE&&this.baseHP<=0)this.endGame(false)}}}
  updateProjectiles(dt){for(const p of [...this.projectiles]){if(!p.target||!p.target.s.active){p.orb.destroy();this.projectiles=this.projectiles.filter(x=>x!==p);continue}const dx=p.target.x-p.x,dy=p.target.y-p.y,d=Math.hypot(dx,dy);const step=p.speed*dt/1000;if(d<=step+9){this.hitEnemy(p.target,p.damage,p.slow,p.slowFactor,p.freezeEvery,p.freezeMs,p.source);this.burst(p.target.x,p.target.y,p.slow?0x72e8ff:0xffdb6d,5);this.beep(p.slow?330:650,.025);p.orb.destroy();this.projectiles=this.projectiles.filter(x=>x!==p)}else{p.x+=dx/d*step;p.y+=dy/d*step;p.orb.setPosition(p.x,p.y)}}}
  hitEnemy(e,dmg,slow=false,slowFactor=.45,freezeEvery=0,freezeMs=0,source=null){e.hp-=dmg;if(slow){e.slow=1900;e.slowFactor=slowFactor;if(freezeEvery&&source){source.freezeHits=(source.freezeHits||0)+1;if(source.freezeHits%freezeEvery===0)e.freeze=Math.max(e.freeze||0,freezeMs)}}e.s.setTintFill(0xffffff);this.time.delayedCall(55,()=>e.s.active&&e.s.clearTint());this.floatText(e.x,e.y-30,`-${dmg}`,'#ffffff')}
