...
const HERO_REGISTRY=window.ORCHARD_HERO_REGISTRY||{normal:[],tree:[]};
const UNIT_DEFS=HERO_REGISTRY.normal.map(h=>({...h}));
const WORLD_TREE_UNIT_DEFS=HERO_REGISTRY.tree.map(h=>({...h}));
...
class BattleScene extends Phaser.Scene{
  ...
  makeTextures(){
    ...
    mk('ironrhino',g=>{g.fillStyle(0x59636b).fillEllipse(47,54,68,48);g.fillStyle(0x7d8990).fillCircle(62,42,26);g.fillStyle(0xc8d0d4).fillTriangle(80,40,95,33,81,50);g.fillStyle(0x262d33).fillCircle(66,38,4);g.fillStyle(0x444d53).fillRoundedRect(20,69,18,18,5).fillRoundedRect(58,69,18,18,5);g.lineStyle(4,0xaab3b8).strokeEllipse(47,54,62,43)});
    mk('skyserpent',g=>{g.lineStyle(15,0x2e8d91).beginPath().moveTo(18,70).cubicBezierTo(32,25,63,82,78,33).strokePath();g.lineStyle(7,0x65d6c9).beginPath().moveTo(19,69).cubicBezierTo(33,30,62,78,77,35).strokePath();g.fillStyle(0x2f718a).fillCircle(78,31,18);g.fillStyle(0xe4fff9).fillCircle(73,28,3).fillCircle(84,28,3);g.fillStyle(0x63e6ff).fillTriangle(79,10,88,24,70,24)});
    ...
    // V16.7: unique, stable field textures for World Tree heroes.
    // These are generated from simple primitives so Safari does not need external assets.
    const eventTex=[['elderowl','🦉'],['spiritstag','🦌'],['crystalspider','🕷️'],['emeraldlizard','🦎'],['stormeagle','🦅'],['moonwolf','🐺'],['ironrhino','🦏'],['skyserpent','🐲'],['ancienttreant','🌲'],['worldcore','🌌']];
    eventTex.forEach(([key,icon])=>{
      g.clear();g.fillStyle(0x163f55,.92).fillCircle(48,48,38);g.lineStyle(5,0x78e6b1,1).strokeCircle(48,48,38);
